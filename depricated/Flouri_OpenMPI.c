...
int main(int argc, char* argv[]) {
    ...
    for (int pair_idx = rank; pair_idx < total_pairs; pair_idx += size) {
        // Convert linear index to sequence pair indices
        int i = 0;
        int j = 1;
        ...
        printf("Process %d computing LCP for sequences %s and %s:\n",
               rank, sequences[i].header, sequences[j].header);
        compute_k_LCP(sequences[i].sequence, sequences[j].sequence, k);
    }
