...
typedef struct {
    ...
    PosLenKey result;     // Result from Rkt_LCS_single_cuda
...
int main(int argc, char* argv[]) {
    ...
    SequenceResult* local_results = (SequenceResult*)malloc(local_count * sizeof(SequenceResult));
    if (!local_results) {
    ...
    for (int i = 0; i < local_count; i++) {
        ...
        // printf("Process %d: Calling Rkt_LCS_single_cuda (k=%d, t=%d, tau=%d, r=%d)\n", 
        //        rank, k, t, tau, r);
               
        // Compute result for this sequence using CUDA implementation
        PosLenKey result = Rkt_LCS_single_cuda(S1, S2_array, num_sequences - 1, k, t, tau, r);
    ...
    if (rank == 0) {
        all_results = (SequenceResult*)malloc(total_results * sizeof(SequenceResult));
    }
    ...
    if (rank == 0) {
        FILE* outfile = fopen("rkt_lcs_gpu_results.txt", "w");
        if (outfile) {
            fprintf(outfile, "Combined results (GPU) with k=%d, t=%d, tau=%d:\n", k, t, tau);
            fprintf(outfile, "===============================================\n\n");
            ...
            fclose(outfile);
            printf("All results written to rkt_lcs_gpu_results.txt\n");
            fflush(stdout);
